...
def create_cnn_model_2(spectrogram_type, path):
    # load pickle file
    ...
    # create models
    models = [CNN.create_model_2(x_trains[i]) for i in range(n)]
    ...
    print(meanAccuracy)
    bestModel = tf.keras.models[most_successful_index].load_model(spectrogram_type + '_CNN2_checkpoint.h5')
    drawConfusionMatrix(x_tests[most_successful_index], y_tests[most_successful_index], bestModel)
    np.save(spectrogram_type + "-CNN2-" + "X_train.npy", x_trains[most_successful_index])
    np.save(spectrogram_type + "-CNN2-" + "Y_train.npy", y_trains[most_successful_index])
    np.save(spectrogram_type + "-CNN2-" + "X_test.npy", x_tests[most_successful_index])
    np.save(spectrogram_type + "-CNN2-" + "Y_test.npy", y_tests[most_successful_index])